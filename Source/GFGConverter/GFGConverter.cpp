    ...
    try
    {
        ...
        if(flags[FAIL_ON_OVERWRITE])
        {
            if(flags[PACK_GFG])
            {
                ...
                outGFGName.replace_extension(fs::path("gfg"));
            ...
            else for(const auto& mesh : parsedMeshes)
            {
                std::string meshPath = SceneRelativePathToAbsolute(mesh.filePath,
                                                                   inScenePath);
                auto gfgPath = fs::path(meshPath).replace_extension(fs::path("gfg"));
        ...
        if(flags[PACK_GFG])
        {
            auto gfgPath = fs::path(outFileName).replace_extension(fs::path("gfg"));
            std::ofstream outFile(gfgPath);
            GFGFileWriterSTL fileWriter(outFile);
            exporter.Write(fileWriter);
            exporter.Clear();
        ...
        if(flags[PACK_GFG])
        {
            ...
            packedPrimNode[NodeNames::TAG] = "gfg";
            // TODO: Change this
