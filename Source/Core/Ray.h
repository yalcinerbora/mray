...
class RayT
{
    ...
    MR_PF_DECL_V explicit RayT(const Vector<3, T>& direction,
                               const Vector<3, T>& position) noexcept;
    MR_PF_DECL_V explicit RayT(const Vector<3, T>[2]) noexcept;

    // Intersections
    MR_PF_DECL
    RayIntersectResult IntersectsSphere(Vector<3, T>& pos, T& t,
                                        const Vector<3, T>& sphereCenter,
                                        T sphereRadius) const noexcept;
    MR_PF_DECL
    RayIntersectResult IntersectsTriangle(Vector<3, T>& baryCoords, T& t,
    ...
    RayIntersectResult IntersectsTriangle(Vector<3, T>& baryCoords, T& t,
                                          ...
                                          bool cullFace = true) const noexcept;
    MR_PF_DECL
    RayIntersectResult IntersectsPlane(Vector<3, T>& position, T& t,
                                       const Vector<3, T>& planePos,
                                       const Vector<3, T>& normal) const noexcept;

    MR_PF_DECL
    RayIntersectResult IntersectsAABB(Vector<2, T>& tOut,
                                      const Vector<3, T>& aabbMin,
                                      const Vector<3, T>& aabbMax,
                                      const Vector<2, T>& tMinMax = CoveringTMM) const noexcept;
    MR_PF_DECL
    RayIntersectResult IntersectsAABB(const Vector<3, T>& min,
                                      const Vector<3, T>& max,
                                      const Vector<2, T>& tMinMax = CoveringTMM) const noexcept;
    MR_PF_DECL
    RayIntersectResult IntersectsAABB(Vector<3, T>& pos, T& t,
