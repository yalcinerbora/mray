                                       ...
                                       const std::array<Vector2, 3> uvs)
{
        ...
        i += kp.TotalSize())
    {
        ...
        isValid &= (soa.indexList[index] == (indices[0] + i));
...
TEST(DefaultTriangle, Load)
{
    ...
    using CountInternal = ItemCountT<PrimBatchKey::Type>;

    std::vector<CountInternal> primCounts;
    primCounts.push_back
    (
        CountInternal
        {
    ...
    // MRay does **not** guarantee that the any order of primitives
    // but we have only one primitive and its id must be zero
    // internally (relative to the group).
    //
    // Thus, just set the key to 0 as well
