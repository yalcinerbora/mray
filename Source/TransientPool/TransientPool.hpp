...
inline void TransientData::Push(Span<const T> data)
{
    if constexpr(EnableTypeCheck)
    {
        if(typeHash != typeid(std::remove_const_t<T>).hash_code())
            throw MRayError("TransientData(Push): Object did not constructed with this type!");
    }
...
inline Span<const T> TransientData::AccessAs() const
{
    ...
    {
        if(typeHash != typeid(std::remove_const_t<T>).hash_code())
            throw MRayError("TransientData(AccessAs): Object did not constructed with this type!");
    }
...
size_t TransientData::Size() const
{
    ...
    {
        if(typeHash != typeid(std::remove_const_t<T>).hash_code())
            throw MRayError("TransientData(Size): Object did not constructed with this type!");
    }
    ...
    if constexpr(EnableTypeCheck)
    {
        if(typeHash != typeid(std::string).hash_code() &&
           typeHash != typeid(std::string_view).hash_code())
            throw MRayError("TransientData(Push): Object did not constructed with "
...
inline std::string_view TransientData::AccessAsString() const
{
    if constexpr(EnableTypeCheck)
    {
        if(typeHash != typeid(std::string).hash_code() &&
           typeHash != typeid(std::string_view).hash_code())
            throw MRayError("TransientData(AccessAsString): Object did not constructed with "
                            "either std::string or std::string_view!");
    }
